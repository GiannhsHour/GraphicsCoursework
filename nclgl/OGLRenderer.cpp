...
void OGLRenderer::UpdateShaderMatrices()	{
	if(currentShader) {
		...
		glUniformMatrix4fv(glGetUniformLocation(currentShader->GetProgram(), "shadowMatrix1"), 1, false, (float*)&shadowMatrix[0]);
		glUniformMatrix4fv(glGetUniformLocation(currentShader->GetProgram(), "shadowMatrix2"), 1, false, (float*)&shadowMatrix[1]);
	}
}
